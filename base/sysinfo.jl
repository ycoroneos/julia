...
function cpu_info()
    ...
    count = Ref{Int32}()
    Base.uv_error("uv_cpu_info",ccall(:uv_cpu_info, Int32, (Ref{Ptr{UV_cpu_info_t}}, Ref{Int32}), UVcpus, count))
...
function uptime()
    uptime_ = Ref{Float64}()
    Base.uv_error("uv_uptime",ccall(:uv_uptime, Int32, (Ref{Float64},), uptime_))
    return uptime_[]
end
